...
### 🔄 부분 완료 기능들 (Release 3)
- **Q&A 게시판**: 완료 (댓글, 비밀글, 검색 포함)
- **파일 업로드**: 기본 구조만 구현, 완전한 기능 필요
- **공지사항 게시판**: 기본 구조만 구현

### ⚠️ 개발 필요 기능들 (Release 4-5)
...
**구현 완료 내용:**
- [x] 공지사항 게시판 엔티티 설계 (Post 엔티티 - NOTICE 타입)
- [x] 공지사항 백엔드 API 구현 (CRUD, 검색, 페이징)
- [x] 관리자 전용 공지사항 작성 권한 구현
- [x] 공지사항 목록 및 상세 조회 기능
- [x] 공지사항 프론트엔드 UI 구현
- [x] 권한 기반 공지사항 수정/삭제 기능
...
**구현된 내용:**
- [x] 파일 업로드 백엔드 API 구조 (FileUploadController)
- [x] 10MB 파일 크기 제한 설정
- [x] 로컬 스토리지 파일 저장 구현
...
**부분 완료 항목:**
- [x] 파일 업로드 기본 구조 (완전한 연동 필요)

**검증 완료 항목:**
- [x] 관리자의 공지사항 등록/수정/삭제 기능
- [x] 사용자의 Q&A 게시글 작성 기능
- [x] 비밀글 권한 제어 기능 (작성자/관리자만 조회)
- [x] 게시판 검색 및 페이징 기능
- [x] 댓글 작성/수정/삭제 기능
- [x] 게시글 조회수 증가 기능
...
| **SFR-003** | 공통: 게시판 | US 3.1, US 3.2, US 3.3, US 3.4 | ✅ 완료 (공지사항/Q&A 완료, 파일업로드 기본구조) |
...
| Release 3 | 2주 | ✅ 완료 | 게시판, 커뮤니티 |
| Release 4 | 3주 | 🔄 개발 필요 | 설문, 평가 시스템 |
| Release 5 | 2주 | 🔄 개발 필요 | 고급 관리, 통계 |
| **총 기간** | **10주** | **68% 완료** | **모든 LMS 기능** |

---

## 다음 Sprint 추천

현재 Release 1-3가 완료된 상태이므로, **Release 4 Sprint 4-1 (설문 시스템)**부터 시작하는 것을 추천합니다.

### 즉시 시작 가능한 작업
1. **설문 엔티티 설계** (Survey, Question, Response)
2. **설문 생성 CRUD API** 개발
3. **다양한 문항 유형** 구현 (5점 척도, 선다형, 단답형)
4. **설문 응답 수집 API** 개발
5. **설문 결과 통계 및 시각화** 개발
